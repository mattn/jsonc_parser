...
#define _JSON_VALUE_MAKE(r, t, i, v) \
...
json_array_new() {
  ...
  a->n = 0;
  a->e = NULL;
  _JSON_VALUE_MAKE(v, JSON_TYPE_ARRAY, array, a);
  return v;
}
...
json_boolean_new(JSON_BOOLEAN b) {
  JSON_VALUE v;
  _JSON_VALUE_MAKE(v, JSON_TYPE_BOOLEAN, boolean, b);
  return v;
}
...
json_number_new(JSON_NUMBER n) {
  JSON_VALUE v;
  _JSON_VALUE_MAKE(v, JSON_TYPE_NUMBER, number, n);
  return v;
}
...
json_object_new() {
  JSON_VALUE v;
  _JSON_VALUE_MAKE(v, JSON_TYPE_OBJECT, object, (JSON_OBJECT) trie_new());
  return v;
}
